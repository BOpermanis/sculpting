...
def icp(A, B, init_pose=None, max_iterations=20, tolerance=0.001):
    ...
    for i in range(max_iterations):
        ...
        dist_mat = distance_matrix(src[:m,:].T, dst[:m,:].T)
        row_ind, col_ind = linear_sum_assignment(dist_mat)
        indices = col_ind
        distances = dist_mat[row_ind, col_ind]

        # print(src.shape, dst.shape, len(indices), dist_mat.shape)
...
# Constants
N = 48                                    # number of random points in the dataset
...
def test_icp():
    from utils import generate_chessboard_in_camera_space

    # Generate a random dataset
    # A = np.random.rand(N, dim)
    A = np.asarray(list(generate_chessboard_in_camera_space()))
    ...
    for i in range(num_tests):
        ...
        # Run ICP
        start = time.time()
        T, distances, iterations, indices = icp(B, A, tolerance=0.000001)
        total_time += time.time() - start
